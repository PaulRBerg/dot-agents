...
7. Keep transaction, bank, exchange, and tax data local. Redact sample values in reports unless raw rows were explicitly requested.
...
1. Inspect the file with `peek.py`. For a no-shape-change edit, save its JSON report; for intentional row/schema changes, record the expected resulting width and invariants.
2. Choose the smallest tool that preserves values and formatting. Avoid pandas unless necessary; if used, load all columns as strings.
...
5. Report paths, row/column effects, validation results, and any formulas or workbook features that could not be preserved.

...
2. For Base (`8453`), Optimism (`10`), Avalanche (`43114`), and BNB Chain (`56`), run `scripts/etherscan-detect-plan.sh`. On a free plan, route to `blockscout-api.md` instead.
...
Verify current support through `https://lb.routeme.sh/chains`, falling back to `https://lb2.routeme.sh/chains`. Otherwise verify the target's `primaryPublicRpc` with `eth_chainId`, then try `target-fallback-rpcs.json` in order. Public RPCs are best-effort and may be rate limited.

## Explorer Links

Use the target row's `explorerUrl` plus `explorer-paths.json`. Verify nonstandard explorers in their UI; Ronin does not reliably follow Etherscan paths and its chain ID collides with a non-target Chainscout entry.
